$(function() {
    ...
    var karyawanList = $('#karyawanList').DataTable({
        ...
        "ajax": {
            "url": base_url+'/karyawan/getKaryawanList',
            "type": "POST",
            "headers": {
        ...
        "columns": [{
            ...
            },
            {
                ...
                "defaultContent": "-",
                render: function(data, type, row) {
                    console.log(row.mappings);
                    if (row.mappings && row.mappings.length > 0) {
                        var mappingsString = row.mappings.map(function(mapping) {
                            return mapping.training.jenis;
                        }).join('<br> ');

                        return "<div class='text-wrap'>" + mappingsString + "</div>";
                    } else {
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    if (row.mappings && row.mappings.length > 0) {
                        var mappingsString = row.mappings.map(function(mapping) {
                            return moment(mapping.training.tgl_sertifikat).format("DD MMMM YYYY");
                        }).join('<br> ');
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    btnEdit += '<a href="'+base_url+'/karyawan/'+data+'" name="btnEdit" data-id="' + data +
    ...
    $(document).on('click', '#btnHapus', function(e, params)
    {
        if(confirm("Apakah yakin ingn menghapus?")){
            var id = $(this).data('id');
            console.log(id);
            var url = base_url+'/karyawan/delete/'+id;
            $.ajax({
                headers:
                {
                    'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content')
